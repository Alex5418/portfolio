      ...
      <Section title="Coursework" subtitle="Courses">
        ...
        <div className="grid gap-2">
          {filtered.map((c) => {
            ...
            return (
              <div key={c.name} className="flex items-center justify-between px-4 py-3 bg-surface border border-border rounded-lg gap-3 flex-wrap">
                <div className="flex items-center gap-2.5 flex-1">
                  <Badge style={{ background: cat.bg, color: cat.text }}>{cat.label}</Badge>
                  <span className="text-[13px] font-medium">{c.name}</span>
                  {'project' in c && c.project && (
                    <span className="text-[10px] text-accent cursor-pointer">🔗 View Project</span>
                  )}
                </div>
